...
def eval_domain(G1, G2, F1, F2, test_loader, **kwargs):
    ...
    with torch.no_grad():
        for data in tqdm(test_loader):
            ...
            confidences_cnn_all.append(
                nn.Softmax(dim=1)(logits_cnn_all[-1]).max(1)[0]
            )
            confidences_vit_all.append(
                nn.Softmax(dim=1)(logits_vit_all[-1]).max(1)[0]
            )
...
def train_labeled_data(config, G1, G2, F1, F2, dset_loaders):
    ...
    ### Set optimizer ###
    optimizer_g1 = optim.SGD(params1, momentum=0.9, weight_decay=0.0005, nesterov=True)
    optimizer_g2 = optim.SGD(params2, momentum=0.9, weight_decay=0.0005, nesterov=True)
    optimizer_f1 = optim.SGD(
        list(F1.parameters()), lr=1.0, momentum=0.9, weight_decay=0.0005, nesterov=True
    )
    optimizer_f2 = optim.SGD(
        list(F2.parameters()), lr=1.0, momentum=0.9, weight_decay=0.0005, nesterov=True
    )
    #####################
    ...
    for step in range(config["adapt_iters"]):
        optimizer_g1 = inv_lr_scheduler(param_lr_g1, optimizer_g1, step, init_lr=0.001)
        optimizer_g2 = inv_lr_scheduler(param_lr_g2, optimizer_g2, step, init_lr=0.01)
        optimizer_f1 = inv_lr_scheduler(param_lr_f1, optimizer_f1, step, init_lr=0.01)
        optimizer_f2 = inv_lr_scheduler(param_lr_f2, optimizer_f2, step, init_lr=0.01)
        ...
        ### Concat Data ###
        labeled_targetw_input = torch.cat(labeled_batch + [inputs_target_w], 0)
