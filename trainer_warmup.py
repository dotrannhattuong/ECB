...
def evaluate(G1, G2, F1, F2, dset_loaders, **kwargs):
    ...
    source_test_result = eval_domain(
        G1, G2, F1, F2, dset_loaders["source_test"], **kwargs
    )
    ...
    # Evaluate model on Target Val in SSDA
    if method == "SSDA":
...
def train_labeled_data(config, G1, G2, F1, F2, dset_loaders):
    ### Some variables ###
    method = config["dataset"]["method"]
    out_file = config["out_file_warmup"]
    config_op = config["optimizer"]
    log_str = f"Method: {method} - File: trainer_warmup.py"
    ...
    ##### Training #####
    for step in range(config["source_iters"]):
        ### Optimizer Scheduler ###
        ...
        if method == "SSDA":
            ...
            labeled_batch = [source_w, target_w]
            labeled_gt = [source_labeled, target_labeled]
        
        else:  # UDA
            labeled_batch = [source_w]
            labeled_gt = [source_labeled]
        #####################################################

        ### Concat Data ###
        labeled_input = torch.cat(labeled_batch, 0)
        ...
        ###### 2. CNN ######
        cnn_logits = F2(G2(labeled_input))
        cnn_loss = ce_criterion(cnn_logits, labeled_gt)
        ...
        if step % config["test_interval"] == config["test_interval"] - 1:
            ...
            if cnn_acc_source >= the_best_acc_cnn_source:
                ...
                if config["save_models"]:
                    write_logs(out_file, f"  -- Saved CNN Branch (G2 + F2) at {config['output_path_warmup']}")
                    torch.save(
                        G2.state_dict(),
                        os.path.join(config["output_path_warmup"], "the_best_G2_pretrained.pth.tar"),
                    )
                    torch.save(
                        F2.state_dict(),
                        os.path.join(config["output_path_warmup"], "the_best_F2_pretrained.pth.tar"),
                    )
            ...
            if vit_acc_source >= the_best_acc_vit_source:
                ...
                # Save model
                if config["save_models"]:
                    write_logs(out_file, f"  -- Saved ViT Branch (G1 + F1) at {config['output_path_warmup']}")
                    torch.save(
                        G1.state_dict(),
                        os.path.join(config["output_path_warmup"], "the_best_G1_pretrained.pth.tar"),
                    )
                    torch.save(
                        F1.state_dict(),
                        os.path.join(config["output_path_warmup"], "the_best_F1_pretrained.pth.tar"),
                    )
